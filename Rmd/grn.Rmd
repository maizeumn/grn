...
\pagebreak
```{r, message=FALSE, echo=FALSE}
fi = '/home/springer/zhoux379/projects/barn/data/01.cfg.xlsx'
...
tp %>%
    kable(format='latex', escape=F, longtable=F, booktabs=T, linesep="",
        format.args = list(big.mark = ","),
        caption = "Table 2. TF knockout mutant RNA-Seq datasets used in this study.") %>%
    kable_styling(latex_options = c("striped", "hold_position"), full_width=F, font_size=9)
```
...
tp = read_tsv(fi) %>%
    select(ID=reg.gid, `\\#_Targets`=max.grp.size, `Supported eQTL study`=qtags, Annotation=note2)
tp %>%
    kable(format='latex', escape=F, longtable=T, booktabs=T,
        format.args = list(big.mark = ","),
        caption = "Table 3. GRN-predicted TFs supported by trans-eQTL hotspots.") %>%
    kable_styling(latex_options = c("striped", "hold_position"), full_width=F, font_size=9)
```
